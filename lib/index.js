...
function apply(ctx, config) {
  ctx.tools.register(defineTool({
    name: "recommend",
    description: "Plugin recommender: find DSH plugins for a need from the marketplace catalog. Actions: search (default) with a free-text query plus optional category and topN; categories (list categories with counts); detail <name> (full catalog entry); similar <name> (nearest alternatives by shared tags/category/description); refresh (fetch the latest catalog from the awesome-dsh-plugin README). Search results carry a scored breakdown and can be deduplicated by repo and diversified across categories. Examples: recommend search \"browser automation with screenshots\"; recommend search \"网页浏览\" category tools; recommend detail dsh-plugin-gate; recommend categories.",
    parameters: {
      query: {
        type: "string",
      ...
      action: {
        type: "string",
        enum: ["search", "categories", "detail", "similar", "refresh"],
      ...
      diversify: {
        type: "boolean",
        description: "Cap results per category so one category cannot fill the list (search / similar).",
      },
      perCategory: {
        type: "integer",
        description: "With diversify, max results per category (default 2).",
      },
      dedupe: {
        type: "boolean",
        description: "Collapse duplicate catalog entries that share a repo url (default true for search).",
      },
    },
    output: {
      schema: {
      ...
      render: (_args, value) => {
        ...
        for (const r of (value.results || [])) {
          ...
          if (r.details && r.details.length) lines.push("      breakdown: " + r.details.map((d) => d.field + ":" + d.term + " +" + d.weight).join(", "));
    ...
    execute: async (args) => {
      ...
      if (action === "similar") {
        const { target, results } = similar(entries, query, { topN: typeof args.topN === "number" && args.topN > 0 ? args.topN : config.topN });
        if (!target) return { action: "similar", ok: false, detail: "no plugin named " + (query || "(empty)") + " in catalog", source, catalogSize: entries.length };
        return {
          action: "similar",
          ok: true,
          detail: "plugins similar to " + target.n,
          source,
          catalogSize: entries.length,
      ...
      const results = rank(entries, terms, {
        category: args.category,
        topN,
        minScore: config.minScore,
        diversify: args.diversify === true,
        perCategory: args.perCategory,
        dedupeByRepo: args.dedupe !== false,
      }).map((r) => ({
        name: r.n,
        url: r.u,
        category: r.c,
